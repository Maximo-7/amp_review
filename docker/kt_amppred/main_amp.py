# Adapted from main_amp.py
# Source: https://github.com/liangxiaodata/AMPpred
# Changes: argparse and torch.save line to save the model
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train ProtT5-based AMP classifier"
    )
...
train_label = pd.read_csv(args.train_tsv,sep='\t')['label'].tolist()
test_label = pd.read_csv(args.test_tsv,sep='\t')['label'].tolist()
...
integrated_model.pretrained_model.save_pretrained(args.output)
torch.save(integrated_model.classifier_model.state_dict(), f'{args.output}/classifier_weights.pth') # added
