# Adapted from MyTrain.py
# Source: https://github.com/lichaozhang2/PLAPD
# Changes: argparse, torch.save line to save the model and
# 'outputs, _, _, _ = model(batch_protein_sequences)' instead of
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train PLAPD AMP classifier"
    )

    parser.add_argument("--train_csv", required=True)
    parser.add_argument("--val_csv", required=True)
    parser.add_argument("--output", type=str, default="my_best_model_without_embedding.pth", help="Output model file")
...
for epoch in range(epochs):
    ...
    for index, batch in enumerate(train_dataloader):
        ...
        optimizer.zero_grad()
        outputs, _, _, _ = model(batch_protein_sequences)
